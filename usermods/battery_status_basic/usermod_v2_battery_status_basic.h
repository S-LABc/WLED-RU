...
class UsermodBatteryBasic : public Usermod 
{
  ...
  public:
    ...
    void addToJsonInfo(JsonObject& root)
    {
      ...
      JsonArray batteryPercentage = user.createNestedArray("Уровень заряда");//Battery level
      JsonArray batteryVoltage = user.createNestedArray("Напряжение АКБ");//Battery voltage

      if (initializing) {
        batteryPercentage.add((nextReadTime - millis()) / 1000);
        batteryPercentage.add(" с");//sec
        batteryVoltage.add((nextReadTime - millis()) / 1000);
        batteryVoltage.add(" с");//sec
        return;
      }

      if(batteryLevel < 0) {
        batteryPercentage.add(F("ошибка"));//invalid
      } else {
      ...
      if(voltage < 0) {
        batteryVoltage.add(F("ошибка"));//invalid
      } else {
        batteryVoltage.add(truncate(voltage, 2));
      }
      batteryVoltage.add(F(" В"));//V
    }
    ...
    */
    ...
    void addToConfig(JsonObject& root)
    {
      ...
      */ 
      ...
      #ifdef ARDUINO_ARCH_ESP32
        battery["Контакт-АКБ"] = batteryPin;                              // usermodparam (pin)
      #endif
      battery["Нижний-порог"] = minBatteryVoltage;           // usermodparam (minBatteryVoltage)
      battery["Верхний-порог"] = maxBatteryVoltage;           // usermodparam (maxBatteryVoltage)
    ...
    bool readFromConfig(JsonObject& root)
    {
      ...
      */
      ...
      #ifdef ARDUINO_ARCH_ESP32
        newBatteryPin     = battery["Контакт-АКБ"] | newBatteryPin; //pin
      #endif
      minBatteryVoltage   = battery["Нижний-порог"] | minBatteryVoltage; //minBatteryVoltage
      //minBatteryVoltage = min(12.0f, (int)readingInterval);
      maxBatteryVoltage   = battery["Верхний-порог"] | maxBatteryVoltage; //maxBatteryVoltage
      //maxBatteryVoltage = min(14.4f, max(3.3f,(int)readingInterval));
      readingInterval     = battery["Интервал-опроса-мс"] | readingInterval; //read-interval-ms
...
// strings to reduce flash memory usage (used more than twice)
const char UsermodBatteryBasic::_name[]         PROGMEM = "Уровень заряда АКБ";//Battery-level
const char UsermodBatteryBasic::_readInterval[] PROGMEM = "Интервал-опроса-мс";//read-interval-ms